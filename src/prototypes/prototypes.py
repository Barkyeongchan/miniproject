...
def region_of_interest(img):
    ...
    pts = np.array([[
        ...
        (int(w*0.45), int(h*ROI_TOP_Y)),
        (int(w*0.55), int(h*ROI_TOP_Y)),
...
def draw_lane_overlay_curve(frame, left_segments, right_segments, prev_left_curve, prev_right_curve):
    ...
    result[10:10+small.shape[0], frame.shape[1]-10-small.shape[1]:frame.shape[1]-10] = cv2.cvtColor(small, cv2.COLOR_GRAY2BGR)
    cv2.rectangle(result, (frame.shape[1]-10-small.shape[1], 10),
                  (frame.shape[1]-10, 10+small.shape[0]), (0,0,0), 2)
